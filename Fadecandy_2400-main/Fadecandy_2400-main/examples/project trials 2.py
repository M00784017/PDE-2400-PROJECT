...
class Ui_MainWindow(object): #GUI created with using QT5 Designer
 ...
 def startTheThread(self, function):
     #runs the chosen function from the argument on a separate thread
 ...
 def star(self):
     #animation of a star poping up randomly 
     while True:
         #endless loop unless stop=1
 ...
 for i in star_list:
     #this for loop to make sure the star is not outside the screen
     if i <0:
...
def on_my_press(key):
    #this function is to stop the animation loop
    ...
    if key.char == 's':
        #if keyboard character is "s" than change the value of stop from 0 to 1
        stop=1 #animations are only running until stop is 0
 
def on_my_release(key):
    #when release keyboard key x the if statement becomes true and will ask you if want to kill the app
    print("key released")
    if key.char == 'x':
...
def myListener():
    #the function listening for keypresses, it will be very useful for the snake animation
